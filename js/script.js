window.addEventListener('DOMContentLoaded', function() {
	...
	tabsParent.addEventListener('click', function(event) { //event.target - ссылка 
		const target = event.target;               //на объект, на который кликнули
    ...
    function getTimeRemaining(endtime) { 
        const t = Date.parse(endtime) - Date.parse(new Date()), //Тут мы получим 
            days = Math.floor( (t/(1000*60*60*24)) ), //кол-во миллисекунд в endtime
    ...
    class MenuCard {
        constructor(src, alt, title, descr, price, parentSelector, ...classes) {
            ...
            this.descr = descr;
            this.price = price;
            this.classes = classes; //Не забыть что тут массив
